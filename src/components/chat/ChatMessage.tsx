...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  return (
    ...
    >
      ...
      {!isUser && (
        <div className="mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-teal-600 to-teal-800 text-white shadow-soft">
      ...
      >
        {isUser ? (
          <div className="rounded-2xl rounded-tr-none bg-[var(--color-primary)] px-4 sm:px-5 py-3.5 text-white shadow-soft">
            ...
            <div className="mt-1.5 flex justify-end text-[11px] text-teal-100/90 font-mono">
              {formattedTime}
            </div>
          </div>
        ) : message.flagged ? (
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 rounded-xl px-2.5 py-1 text-xs font-bold transition-all ${
                    isPlaying
                      ...
                      : "bg-[var(--color-primary-soft)] text-[var(--color-primary)] hover:bg-teal-200/70"
                  }`}
                  title={isPlaying ? t.stop : t.listen}
                >
                  {isPlaying ? <VolumeX className="h-3.5 w-3.5" /> : <Volume2 className="h-3.5 w-3.5" />}
