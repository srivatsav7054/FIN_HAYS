      ...
      <main className="relative z-10 flex flex-1 items-center justify-center px-4 py-10 sm:px-8">
        ...
        >
          <div className="relative hidden overflow-hidden bg-[var(--color-primary)] p-10 text-white lg:block">
            <img src="/images/saahara/women-phone-yellow-03.png" alt="" className="pointer-events-none absolute inset-0 h-full w-full object-cover opacity-25" />
            <div className="absolute -right-20 -top-20 h-64 w-64 rounded-full bg-teal-300/30 blur-3xl animate-float" />
            <div className="absolute -bottom-24 -left-16 h-72 w-72 rounded-full bg-amber-300/25 blur-3xl animate-float-slow" />
            <div className="relative z-10 flex h-full flex-col justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-teal-100">{t.loginEyebrow}</p>
                <h1 className="mt-6 font-display text-5xl font-bold leading-tight">{t.loginHeadline}</h1>
                <p className="mt-5 max-w-sm text-sm leading-7 text-teal-50/85">
                  {t.loginBody}
                </p>
              </div>
              <div className="flex items-center gap-3 text-sm font-semibold text-teal-50">
          ...
          <div className="p-6 sm:p-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button type="submit" className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[var(--color-primary)] px-5 text-sm font-bold text-white shadow-lift transition hover:-translate-y-0.5 hover:bg-teal-800">
                {t.continueSecurely}
